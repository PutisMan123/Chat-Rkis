...
const setUsername = () => { //зарегестрированный пользователь, ему показывается чат
...
const sendMessage = () => { 
  const message = $messageInput.value.trim()// trim  убирает лишние пробелы
...
// сокет дает открыть интерактивную сессию общения между браузером пользователя и сервером // S это нашь сокет// S.on запускаешь стрелочную функцию
S.on('login', (data) => {
  connected = true
